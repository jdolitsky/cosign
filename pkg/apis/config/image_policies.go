...
type ImagePolicyConfig struct {
	...
	Policies map[string]publiccip.ClusterImagePolicy
}
...
func NewImagePoliciesConfigFromMap(data map[string]string) (*ImagePolicyConfig, error) {
	ret := &ImagePolicyConfig{Policies: make(map[string]publiccip.ClusterImagePolicy, len(data))}
	...
	for k, v := range data {
		...
		clusterImagePolicy := &publiccip.ClusterImagePolicy{}
...
func (p *ImagePolicyConfig) GetMatchingPolicies(image string) (map[string][]publiccip.Authority, error) {
	if p == nil {
		return nil, errors.New("config is nil")
	}

	var lastError error
	ret := map[string][]publiccip.Authority{}
