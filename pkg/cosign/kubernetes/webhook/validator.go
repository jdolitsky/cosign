...
func validatePolicies(ctx context.Context, ref name.Reference, kc authn.Keychain, policies map[string][]publiccip.Authority, remoteOpts ...ociremote.Option) (map[string][]oci.Signature, map[string][]error) {
...
func ValidatePolicy(ctx context.Context, ref name.Reference, kc authn.Keychain, authorities []publiccip.Authority, remoteOpts ...ociremote.Option) ([]oci.Signature, []error) {
	// If none of the Authorities for a given policy pass the checks, gather
	// the errors here. If one passes, do not return the errors.
	authorityErrors := []error{}
