...
// ClusterImagePolicy
func NewConfigMap(ns, name, cipName string, cip *publiccip.ClusterImagePolicy) (*corev1.ConfigMap, error) {
...
func CreatePatch(ns, name, cipName string, cm *corev1.ConfigMap, cip *publiccip.ClusterImagePolicy) ([]byte, error) {
	entry, err := marshal(cip)
	if err != nil {
		return nil, err
	}
...
func marshal(spec *publiccip.ClusterImagePolicy) (string, error) {
